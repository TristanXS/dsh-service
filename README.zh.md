...
DSH Service 将 [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) 作为可靠的 macOS 用户级服务运行：支持登录时启动、健康感知的生命周期命令、事务式更新与自动回滚。
...
| 前台 `npx` | 托管服务 |
| --- | --- |
| 在当前终端中运行 | 作为一个 macOS 用户级 LaunchAgent 运行 |
...
DSH Service 当前仅支持 macOS。Linux 和 Windows 支持处于计划中，尚不可用。Homebrew 不是产品依赖；此仓库仅在持续集成（CI）运行器上用它安装 ShellCheck。
...
托管服务不运行 `npx`。安装和更新会将 `@deepseek-ai/dsh` 当前 npm `latest` 版本解析到私有的带版本软件包树中。管理器会记录 Node.js 可执行文件的绝对路径，并通过 `launchd` 调用 DSH 入口点。
...
管理器拥有以下主目录中的路径。表中的 `release_id` 表示生成的带版本目录名。

| 用途 | 路径 |
| --- | --- |
| 命令 | `~/.local/bin/dsh-service` |
| 管理器根目录 | `~/Library/Application Support/dsh-service` |
| 运行器模板 | `~/Library/Application Support/dsh-service/libexec/dsh-service-run` |
| 带版本的发布 | `~/Library/Application Support/dsh-service/releases` |
| 发布目录 | `~/Library/Application Support/dsh-service/releases/release_id` |
| 发布软件包树 | `~/Library/Application Support/dsh-service/releases/release_id/node_modules/` |
| 发布清单 | `~/Library/Application Support/dsh-service/releases/release_id/manifest.env` |
| 发布运行器 | `~/Library/Application Support/dsh-service/releases/release_id/run` |
| 发布完成标记 | `~/Library/Application Support/dsh-service/releases/release_id/.complete` |
| 活跃发布链接 | `~/Library/Application Support/dsh-service/current` |
| 上一发布链接 | `~/Library/Application Support/dsh-service/previous` |
| 激活日志 | `~/Library/Application Support/dsh-service/activation.env` |
| 操作锁 | `~/Library/Application Support/dsh-service/.lock` |
| 服务工作区 | `~/Library/Application Support/dsh-service/workspace` |
| LaunchAgent plist | `~/Library/LaunchAgents/dev.dsh-service.web.plist` |
| 日志目录 | `~/Library/Logs/dsh-service` |
| 标准输出 | `~/Library/Logs/dsh-service/stdout.log` |
| 标准错误 | `~/Library/Logs/dsh-service/stderr.log` |
...
其他进程占用端口 `3080` 时，管理器会拒绝启动或重启。请在不停止进程的情况下识别监听器：
...
DSH 可以使用你的 macOS 用户权限执行代码和 shell 命令。请仅将它用于可信的项目和指令，并在授予敏感数据访问权限前审查命令。
...
唯一的当前后端是一个 macOS 用户级 LaunchAgent。自动化 Bash 语法检查和隔离测试在 macOS 26.5.2（build 25F84）、arm64 上运行。受控的真实冒烟测试在该主机上以 DSH `0.1.0-rc.6` 完成。它验证了安装、状态、重启、同版本更新、停止、启动和卸载，也验证了移除全部管理器拥有的路径以及保留现有 `~/.dsh` 目录。

这些证据不构成通用兼容性保证。Alpha 版本之间不保证兼容；需要时会提供迁移或全新重装说明。

## 反馈与贡献

请通过 [GitHub Issues](https://github.com/TristanXS/dsh-service/issues) 报告可复现的缺陷。请附上 macOS 版本、架构、Node.js 版本、`dsh-service status` 和脱敏后的日志摘录。请勿公开 API 密钥或 `~/.dsh/.credentials.yaml` 的内容。

请通过 [GitHub Discussions](https://github.com/TristanXS/dsh-service/discussions) 提出问题、想法和未来 Linux 或 Windows 平台需求。贡献应保留以上安全和兼容性边界。
